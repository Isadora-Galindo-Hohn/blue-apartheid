...
plot_data_distance <- clean_data %>% filter(year == year_for_distance_plot, !is.na(dist_over_200))

if (nrow(plot_data_distance) > 0) {
  ggplot(plot_data_distance, aes(x = log(income), y = dist_over_200, color = dominent_pop_group)) +
...
plot_data_interrupt <- clean_data %>% filter(year == year_for_interrupt_plot, !is.na(interruption_freq))

if (nrow(plot_data_interrupt) > 0) {
  ggplot(plot_data_interrupt, aes(x = log(income), y = interruption_freq, color = dominent_pop_group)) +
    geom_point(alpha = 0.3) +
    # Use GLM with quasibinomial family for proportion data to keep predictions between 0 and 1
    geom_smooth(method = "glm", method.args = list(family = quasibinomial(link = "logit")), se = FALSE) +
    labs(
...
income_breaks_10pct_raw <- quantile(clean_data$income, probs = seq(0, 1, by = 0.1), na.rm = TRUE)
...
for (i in 1:num_income_intervals) {
  ...
  if (i == num_income_intervals) {
    ...
    income_interval_labels_10pct_new[i] <- paste0("R", lower_bound, "-R", upper_bound)
  }
}
...
income_interval_colors_10pct <- get_greens_palette(length(income_interval_labels_10pct_new))
...
run_model <- function(df, y_var, year) {
  ...
  model_result <- tryCatch(
    ...
    error = function(e) {
      message("Error fitting model for ", y_var, " year ", year, ": ", e$message)
      return(NULL)
    },
    warning = function(w) {
      message("Warning fitting model for ", y_var, " year ", year, ": ", w$message)
...
models_interrupt <- map(years_interrupt, ~ run_model(clean_data %>% filter(year == .x), "interruption_freq", .x))
names(models_interrupt) <- years_interrupt

# Fit models for distance to water source
models_distance <- map(years_distance, ~ run_model(clean_data %>% filter(year == .x), "dist_over_200", .x))
...
summary_interrupt <- summary_interrupt %>%
  ...
  mutate(
    plot_term = case_when(
      str_detect(term, "dominent_pop_group") ~ str_replace(term, "dominent_pop_group", ""),
      TRUE ~ term
    ),
...
summary_interrupt$plot_term <- factor(as.character(summary_interrupt$plot_term), levels = desired_plot_term_levels)
summary_distance$plot_term <- factor(as.character(summary_distance$plot_term), levels = desired_plot_term_levels)
...
ggplot(summary_interrupt, aes(x = factor(year), y = estimate, color = plot_term, group = plot_term)) +
...
ggplot(summary_distance, aes(x = factor(year), y = estimate, color = plot_term, group = plot_term)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_text(aes(label = significance), vjust = -1, size = 3.5) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
...
table_interrupt_data <- summary_interrupt %>%
  mutate(
    ...
    formatted_coefficient = sprintf("%.3f (%.3f)%s", estimate, std.error, significance)
  ) %>%
...
cat("### Table 1: Regression Coefficients for Water Interruption Frequency by Year\n")
...
income_breaks_10pct_raw_maps <- quantile(clean_data$income, probs = seq(0, 1, by = 0.1), na.rm = TRUE)
...
for (i in 1:num_income_intervals_maps) {
  ...
  if (i == num_income_intervals_maps) {
    ...
    income_interval_labels_10pct_new_maps[i] <- paste0("R", lower_bound, "-R", upper_bound)
  }
}

# Define a color palette for income categories for maps
# Re-using get_greens_palette from above
income_interval_colors_10pct_maps <- get_greens_palette(length(income_interval_labels_10pct_new_maps))
names(income_interval_colors_10pct_maps) <- income_interval_labels_10pct_new_maps
...
for (year in years_distance) {
  generate_and_save_map(year, "dist_over_", "Share of Households >200m from Water")
}
